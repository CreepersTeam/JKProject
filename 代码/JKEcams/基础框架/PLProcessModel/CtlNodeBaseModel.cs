...
namespace FlowCtlBaseModel
{
    ...
    public abstract class CtlNodeBaseModel : ILogRequired
    {
       ...
       // protected int taskPhase = 0; //流程步号（状态机）
        ...
        protected List<string> mesProcessStepID = new List<string>();//MES工序ID
      ...
      //  protected ModPsRecordBll modPsRecordBll = new ModPsRecordBll();
        ...
        public List<string> MesProcessStepID { get { return mesProcessStepID; } set { mesProcessStepID = value; } }
        #endregion
        #region 公开接口
        ...
        public string GetRunningTaskDetail()
        {
            ...
            string taskInfo =currentTaskDescribe;
            return taskInfo;
        ...
        public virtual bool BuildCfg(XElement xe,ref string reStr)
        {
            ...
            if(attr != null)
            {
                ...
                this.mesProcessStepID.AddRange(attr.Value.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries));
            }
        ...
        #endregion
        #region 内部功能接口
        public string GetNextStepID(string palletID)
        {
        ...
        protected string GetCurrentStepID(string palletID)
        {
            string strSql = string.Format(@"palletID='{0}' and palletBinded=1 ", palletID);
            List<MesDBAccess.Model.ProductOnlineModel> products = productOnlineBll.GetModelList(strSql);
            ...
            if (products != null && products.Count() > 0)
            {
                return products[0].processStepID;
            }
            else
            {
                return string.Empty;
