...
namespace ProductRecordView
{
    public partial class ProductDataView : BaseChildView
    {
        ...
        private ViewProduct_PSBLL productViewBll = new ViewProduct_PSBLL();
     ...
     //   private ProduceRecordBll recordBll = null;
        ...
        private void OnQuery()
        {
            try
            {
                ...
                {
                    strWhere += "and palletID like '%" + textBox1.Text.Trim() + "%'";
                }

                if (checkBox2.Checked && textBox2.Text.Trim().Length > 0)
                {
                    strWhere += "and batchName like '%" + textBox2.Text.Trim() + "%'";
                }

                if (checkBox3.Checked && textBox3.Text.Trim().Length > 0)
                {
                    strWhere += "and productID like '%" + textBox3.Text.Trim() + "%'";
                }

                strWhere += " order by onlineTime asc";
                ...
                dt.Columns["productCata"].ColumnName = "类别";
                dt.Columns["batchName"].ColumnName = "模组码";
                dt.Columns["palletID"].ColumnName = "工装板码";
                dt.Columns["palletBinded"].ColumnName = "是否绑定";
                dt.Columns["checkResult"].ColumnName = "检测结果";
                dt.Columns["onlineTime"].ColumnName = "上线时间";
                dt.Columns["modifyTime"].ColumnName = "最后更新时间";

                dt.Columns["tag1"].ColumnName = "行";
                dt.Columns["tag2"].ColumnName = "列";
                dt.Columns["tag3"].ColumnName = "层";

                this.dataGridView2.DataSource = dt;
                this.dataGridView2.Columns["最后更新时间"].Width = 150;
                this.dataGridView2.Columns["上线时间"].Width = 150;
                ...
                this.dataGridView2.Columns["stationID"].Visible = false;
                this.dataGridView2.Columns["是否绑定"].Visible = false;
                this.dataGridView2.Columns["检测结果"].Visible = false;
        ...
        private void OnFill()
        {
            ...
            //        productModel.processStepID = "PS-41";
            ...
            //{
            //    logRecorder.AddDebugLog(this.Text, string.Format("二次装载失败，{0},返回错误{1},原始发送数据：{2}", palletID, reStr, sndStr));

            //}
            //else
            //{
            //    logRecorder.AddDebugLog(this.Text, string.Format("二次装载成功，{0}", palletID));
            //}
