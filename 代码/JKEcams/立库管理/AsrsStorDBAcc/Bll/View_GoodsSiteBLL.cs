...
namespace AsrsStorDBAcc.BLL
{
    ...
    public partial class View_GoodsSiteBLL
    {
        ...
        public List<View_GoodsSiteModel> GetGsByHouseAndArea(string houseName, string storeHouseAreaName, bool isFull)
        {
            string sqlStr = "1=1";
            if (storeHouseAreaName != "所有")
            {
            ...
            if(isFull == true)
            {
                sqlStr += " and GoodsSiteStatus = '满位' and GoodsSiteTaskStatus ='完成'";
            }
            else
            {
                sqlStr += " and GoodsSiteStatus = '空闲' and GoodsSiteTaskStatus ='完成'";
            }
            List<View_GoodsSiteModel> viewList = GetModelList(sqlStr);
            return viewList;
        }
